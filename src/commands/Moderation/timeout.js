...
module.exports.run = async (interaction, utils, guild, member) =>
{
    try
    {
        ...
        if(!time) return interaction.reply({ content: "Please provide a valid Time to Timeout", ephemeral: true })

        const timeout = await target.timeout(time, reason)

        await interaction.reply({ content: `${emojis.success} successfully timeouted ${target}`, ephemeral: true });
...
module.exports.data = new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName("target").setDescription("Select the User to timeout").setRequired(true))
    .addStringOption(option => option.setName('reason').setDescription("Provide a reason to timeout").setRequired(true))
    .addNumberOption(option => option.setName('time').setDescription("Select the Time for the Timeout").setRequired(true))