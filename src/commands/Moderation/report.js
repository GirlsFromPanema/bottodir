...
module.exports.run = async (interaction, utils) =>
{
    try
    {
        ...
        if(!guildQuery) {
            return interaction.reply({ content: `${emojis.error} | This guild has no Report System setup.`, ephemeral: true })
        }
        ...
        return interaction.reply({
        content:
        `${emojis.error} | I cannot warn this user as their highest role is higher than mine or I have the same highest role as them.`,
        ephemeral: true,
        });
        ...
        if(guildQuery) {
            ...
            interaction.reply({ content: `${emojis.success} | Successfully reported ${target} for: ${reason}`, ephemeral: true });
        }
    }
    catch (err)
    {
...
module.exports.data = new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName("target").setDescription("Provide the user you want to report.").setRequired(true))
    .addStringOption(option => option.setName("reason").setDescription("Provide the reason for the ban, add proofs via Imgur.").setRequired(true))
